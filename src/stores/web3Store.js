...
export const useWeb3Store = defineStore('web3', () => {
  ...
  const leaderboardAddress = computed(() => (chainId.value ? BURN_LEADERBOARD_ADDRESS[chainId.value] : null));
  ...
  const loadTokenInfo = async () => {
    ...
    try {
      ...
      tokenContract.value = new ethers.Contract(tokenAddress.value, tokenAbi, signer);
  ...
  const burnTokens = async (amount) => {
    ...
    try {
      ...
      const contract = new ethers.Contract(tokenAddress.value, tokenAbi, signer);
  ...
  const initLeaderboardContract = async () => {
    ...
    try {
      ...
      // Create contract instance
      leaderboardContract.value = new ethers.Contract(leaderboardAddress.value, BURN_LEADERBOARD_ABI, signer);
  ...
  const burnTokensWithLeaderboard = async (amount) => {
    ...
    try {
      ...
      const tokenContract = new ethers.Contract(tokenAddress.value, tokenAbi, signer);

      // Create a fresh leaderboard contract instance with the current signer
      const leaderboardContractInstance = new ethers.Contract(leaderboardAddress.value, BURN_LEADERBOARD_ABI, signer);
      ...
      // Log the amount being approved/burned for debugging
      console.log('Amount to burn (in wei):', amountToSend.toString());
      console.log('Leaderboard address:', leaderboardAddress.value);
      ...
      try {
        ...
        // Wait differently - don't use the wait() method directly
        status.value = 'Waiting for approval confirmation...';

        // Option 1: Manual polling for receipt
        let approvalReceipt = null;
        for (let attempts = 0; attempts < 20 && !approvalReceipt; attempts++) {
          try {
            approvalReceipt = await ethersProvider.getTransactionReceipt(approveTx.hash);
            if (approvalReceipt) break;
          } catch (e) {
            console.log('Still waiting for approval receipt, attempt:', attempts);
          }
        ...
        if (!approvalReceipt) {
          throw new Error('Approval transaction was not mined within the timeout period');
        }

        console.log('Approve transaction confirmed with receipt:', approvalReceipt);
      } catch (approveError) {
        console.error('Error approving tokens:', approveError);
        status.value = `Approval error: ${approveError.message}`;
        return false;
      }

      // Now burn the tokens - check contract interface first
      status.value = 'Burning tokens...';

      try {
        // Call the burn function
        const burnTx = await leaderboardContractInstance.burnTokens(amountToSend, {
          value: 0, // Explicitly set value to 0 to prevent sending native tokens
        });

        console.log('Burn transaction submitted:', burnTx);
        status.value = 'Transaction submitted, waiting for burn confirmation...';

        // Use the same manual polling approach for the burn transaction
        let burnReceipt = null;
        for (let attempts = 0; attempts < 20 && !burnReceipt; attempts++) {
          try {
            burnReceipt = await ethersProvider.getTransactionReceipt(burnTx.hash);
            if (burnReceipt) break;
          } catch (e) {
            console.log('Still waiting for burn receipt, attempt:', attempts);
          }
          // Wait 2 seconds before trying again
          await new Promise((resolve) => setTimeout(resolve, 2000));
        }
        ...
        if (!burnReceipt) {
          throw new Error('Burn transaction was not mined within the timeout period');
        }

        console.log('Burn transaction confirmed with receipt:', burnReceipt);
        ...
        console.error('Detailed burn error:', burnError);

        // Check for specific error types
        if (burnError.code === 'BAD_DATA' && burnError.value && burnError.value.result === null) {
          status.value = 'Error: Transaction failed on the blockchain. This may be due to insufficient gas, contract limitations, or network issues.';
        } else {
          status.value = `Burn error: ${burnError.message}`;
        }
        return false;
      }
    } catch (error) {
